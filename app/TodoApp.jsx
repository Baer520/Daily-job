...
const FILTERS = ['全部', '待处理', '已完成', '近一月历史', '今日', '已逾期'];
const PRIORITIES = {
  low: '低',
  normal: '中',
  high: '高',
  ...
  const visibleTodos = useMemo(() => {
    return todos
      .filter((todo) => {
        ...
        if (filter === '近一月历史') return isRecentHistory(todo);
      ...
      .sort((a, b) => {
        ...
        if (filter === '近一月历史') {
          return new Date(getHistoryTime(b)).getTime() - new Date(getHistoryTime(a)).getTime();
        }
        if (!a.dueAt) return 1;
        if (!b.dueAt) return -1;
  ...
  return (
    <main className="app-shell">
      ...
      <section className="summary-grid" aria-label="待办统计">
        ...
        <div>
          <span>近一月历史</span>
          <strong>{stats.recentHistory}</strong>
        </div>
                  ...
                  <div className="todo-content">
                    ...
                    <div className="meta-row">
                      ...
                      {todo.completed && (
                        <span>
                          <History size={14} />
                          完成于 {formatDateTime(getHistoryTime(todo))}
                        </span>
                      )}
